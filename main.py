...
def _icon_texture(kind):
    """Loads (and caches) the pre-rendered line-icon PNG for `kind`. Icons
    are white silhouettes on a transparent background so ToolIcon can tint
    them to any color via Color()+Rectangle(texture=...) modulation."""
    tex = _ICON_TEXTURE_CACHE.get(kind)
    if tex is None:
        tex = CoreImage(os.path.join(ICONS_DIR, "%s.png" % kind)).texture
        _ICON_TEXTURE_CACHE[kind] = tex
    return tex
...
TOOL_CATALOG = [
    ...
    ("Speed", "speed", "speed", "Velocidade de download da internet"),
    ("Portas", "ports", "ports", "Verifique portas TCP abertas"),
    ("Rede", "scan", "scan", "Descubra hosts ativos na rede"),
    ("Shell", "terminal", "shell", "Comandos de shell, sem root"),
...
class ToolIcon(Widget):
    """Small line-icon: a pre-rendered white silhouette (icons/<kind>.png,
    Feather-style, anti-aliased) tinted at runtime via Color()+Rectangle
    texture modulation. `kind` selects which PNG to load."""
    ...
    def __init__(self, kind, color=None, **kwargs):
        ...
        self._texture = _icon_texture(kind)
        self.bind(pos=self._redraw, size=self._redraw)
    ...
    def _redraw(self, *_):
        ...
        with self.canvas:
            ...
            Rectangle(texture=self._texture, pos=pos, size=(side, side))
...
class NavButton(ButtonBehavior, FloatLayout):
    """Tab button: icon above a label, both muted when inactive and tinted
    accent when active -- no background chip, matching the reference nav
    bar (which only recolors icon/text, never adds a highlight box). The
    icon+label pair is a fixed-size block centered in the full-height tap
    area, so it stays vertically centered no matter how tall the nav bar
    is (rather than stretching the label to fill the leftover space)."""

    def __init__(self, text, icon_kind, on_press_cb, **kwargs):
        super().__init__(**kwargs)
        content = BoxLayout(
            orientation="vertical", spacing=dp(4),
            size_hint=(None, None), size=(dp(52), dp(38)),
            pos_hint={"center_x": 0.5, "center_y": 0.5},
        )
        self.icon = ToolIcon(
            icon_kind, color=TEXT_MUTED,
...
class ToolTile(RoundedBG, ButtonBehavior, BoxLayout):
    """Square tile used in the Home dashboard's tool grid: a flat line-icon
    plus a label. Tapping navigates straight to that tool's screen. The
    "Todas" tile passes muted=True to read as a secondary "see more" tile
    instead of a regular accent-colored tool."""

    def __init__(self, label, icon, on_press_cb, muted=False, **kwargs):
        kwargs.setdefault("orientation", "vertical")
        kwargs.setdefault("size_hint_y", None)
        ...
        icon_color = TEXT_SECONDARY if muted else ACCENT
        text_color = TEXT_SECONDARY if muted else TEXT

        self.add_widget(ToolIcon(
            kind=icon, color=icon_color,
            size_hint=(None, None), size=(dp(20), dp(20)), pos_hint={"center_x": 0.5},
        ))

        text_label = Label(
            text=label, color=text_color, font_size="12sp", halign="center", valign="middle",
...
class MoreListItem(RoundedBG, ButtonBehavior, BoxLayout):
    ...
    def __init__(self, label, sub, icon, on_press_cb, **kwargs):
        ...
        badge_wrap.add_widget(ToolIcon(
            kind=icon, color=ACCENT, size_hint=(0.5, 0.5), pos_hint={"center_x": 0.5, "center_y": 0.5},
        ))
...
class MoreSheet(FloatLayout):
    ...
    def on_touch_down(self, touch):
        # Widget.on_touch_down swallows any touch inside its bounds while
        # disabled=True, even though this sheet covers the full screen --
        # that would block clicks on everything underneath it whenever it's
        # closed. Let touches pass through instead of consuming them.
        if self.disabled:
            return False
...
class SpeedGauge(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        sub_row.add_widget(ToolIcon(
            ...
            # "MBPS" has no descenders, so Kivy's text valign="middle" (which
            # centers using the font's full ascent+descent) renders it a
            # touch above the box's geometric center; nudge the icon up to
            # match the text's actual optical center instead of the box's.
...
class SpeedtestScreen(Screen):
    def __init__(self, **kwargs):
        ...
        # FloatLayout (not BoxLayout) -- a horizontal BoxLayout only honors
        # pos_hint on the cross (vertical) axis and packs the main axis
        # sequentially from the left, so center_x here would be silently
        # ignored and the gauge would sit flush-left instead of centered.
        gauge_wrap = FloatLayout(size_hint_y=None, height=dp(220))
        self.gauge = SpeedGauge(pos_hint={"center_x": 0.5, "center_y": 0.6})
...
class NetworkScanScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.refresh_btn = SmallIconButton(icon_widget=ToolIcon(
            kind="refresh", color=ACCENT, size_hint=(0.55, 0.55),
            pos_hint={"center_x": 0.5, "center_y": 0.5},
        ))
...
class SSHScreen(Screen):
    ...
    def _show_terminal(self):
        ...
        dot = Widget(size_hint=(None, None), size=(dp(9), dp(9)), pos_hint={"center_y": 0.5})
        with dot.canvas:
            Color(*SUCCESS)
...
class ActivityRow(RoundedBG, BoxLayout):
    ...
    def __init__(self, title, subtitle, color=SUCCESS, **kwargs):
        ...
        dot = Widget(size_hint=(None, None), size=(dp(8), dp(8)), pos_hint={"center_y": 0.5})
        with dot.canvas:
            Color(*color)
            dot_ellipse = Ellipse(pos=dot.pos, size=dot.size)
        dot.bind(pos=lambda w, *_: setattr(dot_ellipse, "pos", w.pos))
...
class HomeScreen(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        grid.add_widget(ToolTile("Todas", "more", on_press_cb=self._open_all_tools, muted=True))
...
class MaxApp(App):
    ...
    def _build_ui(self):
        ...
        nav_bar = BoxLayout(size_hint_y=None, height=dp(66))
        ...
        self.nav_buttons = []
        nav_bar.add_widget(self._make_nav_button("Início", "home", "home", lambda: self.switch_screen("home")))
        nav_bar.add_widget(self._make_nav_button("Ping", "ping", "ping", lambda: self.switch_screen("ping")))
        nav_bar.add_widget(self._make_nav_button("Wi-Fi", "wifi", "wifi", lambda: self.switch_screen("wifi")))
        nav_bar.add_widget(self._make_nav_button("Speed", "speed", "speed", lambda: self.switch_screen("speed")))
        nav_bar.add_widget(self._make_nav_button("Mais", "more", "more", self.toggle_more_sheet))
    ...
    def _make_nav_button(self, label_text, key, icon_kind, on_press_cb):
        btn = NavButton(label_text, icon_kind, on_press_cb=on_press_cb)
